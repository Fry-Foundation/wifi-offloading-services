...
UbusResponse *ubus_client_call(UbusClient *client, const char *service_name, 
    ...
    ret = ubus_invoke(client->ctx, service_id, method_name, args, 
                      sync_call_callback, &response_ptr, client->timeout_ms);
...
const char *ubus_response_get_string(UbusResponse *response, const char *key) {
    ...
    blobmsg_for_each_attr(attr, response->data, rem) {
        if (blobmsg_type(attr) == BLOBMSG_TYPE_STRING && 
            strcmp(blobmsg_name(attr), key) == 0) {
...
bool ubus_response_get_bool(UbusResponse *response, const char *key, bool default_value) {
    ...
    int rem;
    blobmsg_for_each_attr(attr, response->data, rem) {
        if (blobmsg_type(attr) == BLOBMSG_TYPE_INT8 && 
            strcmp(blobmsg_name(attr), key) == 0) {
